...
void do_user_external_force(rvec x[], rvec f[], real indices[], real params[],
                            ...
                            t_mdatoms *mdatoms, t_commrec *cr)
{
    int	       ii, i, ndx_l, ndx_h, indx;
    double     offset, f_com, z;
	...
	offset = sim_mean[ii] - exp_mean[0]; // The offset aligns the two densities for the potential
        if (abs(offset) < 0.5)
        {
            offset = 0.0; // If the offset is less than 5 A, don't apply an offset force and take 
        }                 // the experimental density in its actual location, no offset.
        //f_com  = -1.0*scale[1]*offset; // The force of the offset
        f_com = -1.0*scale[1]*offset/(ndx_h-ndx_l+1); // The force of the offset
        ...
        for (i=0; i<homenr; i++)
        {
            ...
            {
                z = x[i][2];
                // Check the pbc on the coordinate
                if (z < params[0])
                {
                    z += z_bbox;
                }
                // Simulation density contribution
                indx = floor(( z - params[0] ) / params[2]); // sim_dens index
                f[i][2] += scale[0]*(sim_dens[ii][indx]-sim_dens[ii][indx+1]) / params[2];
                // Experimental density contribution
                indx = floor(( z - offset - params[0] ) / params[2]); // exp_dens index
                // Only try to access the exp density if the index falls within the array
                if ( (indx >= 0) && (indx < (params[1]-params[0])/params[2]) )
                {
                    f[i][2] += -1.0*scale[0]*(exp_dens[indx]-exp_dens[indx+1]) / params[2];
                }

                // Offset force biasing toward experimental mean
                f[i][2] += f_com;
	    }
        }
    }
              ...
              int flags)
{
    ...
    do_user_external_force(x, f, inputrec->pot_indices, inputrec->pot_params,
                           ...
                           inputrec->sim_dens, inputrec->sim_mean, inputrec->z_bbox, mdatoms, cr);
